...
function formatCurrency(amount) {
    ...
    if (Number.isFinite(value)) return `Br ${value.toFixed(2)}`;

    const parsed = parseFloat(String(amount).replace(/[^0-9.]/g, ''));
    return `Br ${(Number.isFinite(parsed) ? parsed : 0).toFixed(2)}`;
}
...
async function loadDashboard() {
    try {
        ...
        const earningsValue = earnings?.today_earnings ?? earnings?.today ?? earnings?.amount ?? 0;
        document.getElementById('todayEarnings').textContent = formatCurrency(earningsValue);
    ...
    container.innerHTML = parkings.map(parking => `
        <div class="parking-item">
            ...
            <div class="parking-details">
                ...
                <div class="parking-detail">
                    <strong>Price:</strong> Br ${parking.price_per_hour}/hour
                </div>
            </div>
            ...
            <div class="session-details">
                ...
                ${session.current_cost ? `
                    <div class="detail-item">
                        ...
                        <span class="detail-value">Br ${session.current_cost}</span>
                    </div>
                ` : ''}
            </div>
